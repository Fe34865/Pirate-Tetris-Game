...
chest = pygame.image.load('./imagens/score-chest.png')
skull = pygame.image.load('./imagens/skull.png')
    ...
    for row in range(20):
        for collumn in range(12):
            if grid[row][collumn] == 1:
                ...
                pygame.draw.rect(
                    screen, GRAY, (x_block, y_block, block_size, block_size))


def createPieces(x, y):
    I = [[RED, [x, y], [x, y + block_size], [x, y + block_size * 2], [x, y + block_size * 3]],
         [RED, [x - block_size, y + block_size], [x, y + block_size], [x + block_size, y + block_size],
    ...
    J = [[BEIGE, [x, y], [x, y + block_size], [x, y + block_size * 2], [x - block_size, y + block_size * 2]],
         ...
         [BEIGE, [x, y + block_size], [x - block_size, y + block_size], [x + block_size, y + block_size * 2],
          [x + block_size, y + block_size]]]

    L = [[BROWN, [x, y], [x, y + block_size], [x, y + block_size * 2], [x + block_size, y + block_size * 2]],
         [BROWN, [x, y + block_size], [x - block_size, y + block_size],
          [x + block_size, y + block_size], [x - block_size, y + block_size * 2]],
         [BROWN, [x, y], [x, y + block_size], [x, y + block_size * 2], [x - block_size, y]],
         [BROWN, [x, y + block_size], [x - block_size, y + block_size], [x + block_size, y + block_size],
          [x + block_size, y]]]
...
def Score():
    ...
    screen.blit(chest, (530, 205))
    screen.blit(score_text, (560, 200))


def drawNextShape(pieces, num_piece):
    shape_text = font.render('Próxima peça:', 1, YELLOW)
    screen.blit(shape_text, (520, 300))

    drawPieces(pieces, num_piece - 1)
